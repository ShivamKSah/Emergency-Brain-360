...
const ArchiveView: React.FC<ArchiveViewProps> = ({ allPatients, onBackToDashboard }) => {
  return (
    <div className="p-4 lg:p-6 max-w-[100rem] mx-auto">
        <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-slate-100">Patient Archive ({allPatients.length})</h2>
            <button
        ...
        <div className="bg-slate-800/50 rounded-lg shadow-lg border border-slate-700/50 overflow-hidden">
                                ...
                                <th scope="row" className="px-6 py-4 font-medium text-slate-200 whitespace-nowrap">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {patient.status === 'DISCHARGED' && (
                                            <span 
                                                className="bg-sky-500/20 text-sky-300 text-xs font-bold px-2 py-0.5 rounded-full"
                                                title="Discharged"
                                            >
                                                D
                                            </span>
            ...
            {allPatients.length === 0 && (
                <p className="text-center py-8 text-slate-500">No patients in the archive.</p>
            )}
        </div>
    </div>
  );
